...
"[project]/src/app/api/user/sales/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
;
;
function getCashierAggregate(transactions) {
    ...
    return {
        ...
        top: Array.from(salesMap.values()).sort((a, b)=>b.total - a.total).slice(0, 3)
    };
}
function getCashiersSalesList(monthTransactions, dayTransactions) {
    ...
    return Array.from(monthMap.values()).map((u)=>({
            userId: u.userId,
            name: u.name,
...
function getStats(transactions) {
    const cash = transactions.filter((t)=>t.paymentMethod === 'CASH').reduce((sum, t)=>sum + t.amount, 0);
    const card = transactions.filter((t)=>t.paymentMethod === 'CARD').reduce((sum, t)=>sum + t.amount, 0);
    return {
...
async function GET() {
    try {
        ...
        const monthTransactions = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].transaction.findMany({
            where: {
                userId: user.id,
                createdAt: {
                    gte: monthStart
        ...
        const dayTransactions = monthTransactions.filter((t)=>new Date(t.createdAt).getTime() >= dayStart.getTime());
        const monthAllTransactions = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].transaction.findMany({
            where: {
                createdAt: {
                    gte: monthStart
                }
        ...
        const dayAllTransactions = monthAllTransactions.filter((t)=>new Date(t.createdAt).getTime() >= dayStart.getTime());
...
//# sourceMappingURL=%5Broot-of-the-server%5D__095d67d5._.js.map